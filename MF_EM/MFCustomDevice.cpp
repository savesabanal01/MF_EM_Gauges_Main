...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig, bool configFromFlash)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "FFGauge") == 0)
        _customType = FFGAUGE_DEVICE;
    if (strcmp(parameter, "ITTGauge") == 0)
    ...
    if (_customType == FFGAUGE_DEVICE) {
        ...
        ********************************************************************************** */
        ...
        _myFFGaugedevice = new (allocateMemory(sizeof(FFGauge))) FFGauge(_pin1, _pin2);
        _myFFGaugedevice->attach(Parameter1, Parameter2);
        ...
        _myFFGaugedevice->begin();
        _initialized = true;
    }  else if (_customType == ITTGAUGE_DEVICE) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(FFGauge))) {
        ...
        ********************************************************************************** */
        // In most cases you need only one of the following functions
        // depending on if the constuctor takes the variables or a separate function is required
        _myITTGaugedevice = new (allocateMemory(sizeof(ITTGauge))) ITTGauge(_pin1, _pin2);
        _myITTGaugedevice->attach(Parameter1, Parameter2);
        // if your custom device does not need a separate begin() function, delete the following
        // or this function could be called from the custom constructor or attach() function
        _myITTGaugedevice->begin();
        _initialized = true;
        ...
        ********************************************************************************** */
        // In most cases you need only one of the following functions
        // depending on if the constuctor takes the variables or a separate function is required
        _myNPGaugedevice = new (allocateMemory(sizeof(NPGauge))) NPGauge(_pin1, _pin2);
        _myNPGaugedevice->attach(Parameter1, Parameter2);
        // if your custom device does not need a separate begin() function, delete the following
        // or this function could be called from the custom constructor or attach() function
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == FFGAUGE_DEVICE) {
        _myFFGaugedevice->set(messageID, setPoint);
    } else if (_customType == ITTGAUGE_DEVICE) {
